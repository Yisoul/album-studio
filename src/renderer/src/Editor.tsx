    ...
    <div className="inspector">
      ...
      {props.layer.type === 'text' ? <section className="inspector-section">
        ...
        <div className="text-format-grid">
          <div className="field-group"><span>字号</span><div className="font-size-stepper"><button type="button" onClick={() => props.onUpdateText(text, { fontSize: Math.max(8, numberStyle(style.fontSize, 48) - 2) })}>−</button><input type="number" min="8" max="240" value={numberStyle(style.fontSize, 48)} onChange={(event) => props.onUpdateText(text, { fontSize: Math.max(8, Math.min(240, Number(event.target.value))) })} /><button type="button" onClick={() => props.onUpdateText(text, { fontSize: Math.min(240, numberStyle(style.fontSize, 48) + 2) })}>＋</button></div></div>
          <div className="field-group"><span>颜色</span><div className="text-color-control"><input type="color" value={stringStyle(style.color, '#111111')} onChange={(event) => props.onUpdateText(text, { color: event.target.value })} /><div className="color-swatches">{['#111111', '#ffffff', '#bf5f3c', '#2563eb', '#16a34a', '#dc2626'].map((color) => <button type="button" key={color} title={color} style={{ background: color }} className={stringStyle(style.color, '#111111').toLowerCase() === color ? 'active' : ''} onClick={() => props.onUpdateText(text, { color })} />)}</div></div></div>
        </div>
