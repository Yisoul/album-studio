...
- 图库按实际子文件夹分组，可多选文件夹筛选，也可切到“全部图片”；每页 120 张自动加载并支持多种排序。
- 没有可用原图的记录默认不进入图库。
- 双击照片可打开大图并进入全屏；选图器同样支持文件夹筛选、分页和双击预览。
- 编辑器支持单击选中、Ctrl 多选、空白处或 Esc 取消选中，并可删除、替换选中图片图层。
...
安装包输出到 `dist\Album-Studio-0.2.0-Setup.exe`。仓库内的打包配置使用本地 Electron 分发，避免打包时重复下载。
